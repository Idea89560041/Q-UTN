...
class h5Loader(Dataset):
    ...
    '''
    def __init__(self, folder, is_train=True, dir_group=2, pad_size=160):
    ...
    def __getitem__(self, item):
        ...
        for i in range(self.dir_group):
            offset_w = (pad_size - w) // 2
            offset_h = (pad_size - h) // 2
            pad_xs = np.zeros((pad_size, pad_size, 1))
            pad_xs[offset_w:offset_w + w, offset_h:offset_h + h] = b0_list[i]
            b0_list[i] = pad_xs

            pad_ys = np.zeros_like(pad_xs)
            pad_ys[offset_w:offset_w + w, offset_h:offset_h + h] = dwi_list[i]
            dwi_list[i] = pad_ys

            pad_t2 = np.zeros_like(pad_xs)
            pad_t2[offset_w:offset_w + w, offset_h:offset_h + h] = t2_list[i]
            t2_list[i] = pad_t2

            pad_t1 = np.zeros_like(pad_xs)
            pad_t1[offset_w:offset_w + w, offset_h:offset_h + h] = t1_list[i]
            t1_list[i] = pad_t1
